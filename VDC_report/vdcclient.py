...
if __name__ == "__main__":
    if len(sys.argv)!=2:
        print 'num of argv=',len(sys.argv),', need 2.'
        print 'Usage:'
    ...
    while request_md5_num >0:
        ...
        while True:
            try:
                selected_list=hbase_query(md5_list)
                if selected_list!=None:
                    break
            except Exception,e:
                print e
                print 'repeat hbase_query()'
                time.sleep(3)

        print 'select ratio = %d/%d'%(len(selected_list),len(md5_list))
        real_md5_num = len(selected_list)
        if real_md5_num > 0:        
            insert_to_mysql(selected_list)
        request_md5_num -= real_md5_num
        print 'current left md5 = ',request_md5_num
        #time.sleep(1)
