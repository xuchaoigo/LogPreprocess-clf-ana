...
def do_routine(WORKING_DIR,SAMPLE_DIR,db_ip,db_name,model_name):
    orig_dir = SAMPLE_DIR+'Sample/orig/'
    v2orig_dir = SAMPLE_DIR+'Sample/v2orig/'    
    ...
    refiner_py = '%srefiner_v3.py'%pre_script_dir
    ...
    cmd_refine = 'python %s %s %s %s 12'%(refiner_py, orig_dir, v2orig_dir, log_dir)
    ...
    if os.path.exists(v2orig_dir):
        shutil.rmtree(v2orig_dir)
...
if __name__ == '__main__':
    ...
    date_list = ['2015_03_23']

    db_ip = 'nj02-sw-kvmserver01.nj02'
    db_name = 'selected_iter_g_10w_chi2'
    model_name = 'iter_g_10w_chi2.uniq.final.new'
    #import_log_orig(SRC_DIR,SAMPLE_DIR,date_list)
    do_routine(WORKING_DIR,SAMPLE_DIR,db_ip,db_name,model_name)
    #export_log_orig(SAMPLE_DIR,DST_DIR,date_list)
