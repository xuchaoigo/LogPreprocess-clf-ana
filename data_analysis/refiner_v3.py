...
def convert_v3_to_v2(rawdata_file,v2data_file):
    ...
    for line in file_in:
        try:
                ...
                #extract target_str,(SECTION)target_process
                ...
                if tmap[api_id]==0:
                    ...
                    target_str = api_param_list[tmap[api_id]-1]
                
                if api_id==65546 or api_id==65547:
                    if major_operation == 'THREAD':
                        target_str = api_param_list[3-1]       
        ...
        while 1:
            try:            
                        ...
                        if ParamList[0]== 'SECTION':
                                ...
                                print 'valid  section, file= ',PathList[-1]
                                print ParamList
                                del ParamList[3]
                                print ParamList
                        else:
                        ...
                        if SamplePath == '':
                            if len(ParamList) >= 4:
                                ...
                                if len(sample_name)==36:#confirm this line actually contains md5
                                    ...
                                    #NOTE: white collect sample,sample_name != md5. karlxu 2015_04_17:
                                    SamplePath = ParamList[3]
                                    SampleName = sample_name
                                    #continue
...
def process_file(rawdata_file):
    ...
    v2data_file = g_top_v3v2_dir+tail_path
...
def parallel_process_pool(g_top_rawdata_dir,g_top_v3v2_dir,g_top_output_dir,num_tasks):
    file_list = []
    for root,dirs,files in os.walk(g_top_rawdata_dir):
        for name in files:
            ...
            v2data_file = g_top_v3v2_dir+tail_path
            v2data_dir = v2data_file[:v2data_file.rfind('/')]
            output_file = g_top_output_dir+tail_path
            output_dir = output_file[:output_file.rfind('/')]
            if not os.path.exists(v2data_dir):
                os.makedirs(v2data_dir)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 5:
        print 'Usage:'
        print sys.argv[0],' rawdata_dir v3v2_dir log_dir num_tasks'
        sys.exit()

    print 'refiner start'
    g_top_rawdata_dir = sys.argv[1]
    g_top_v3v2_dir = sys.argv[2]
    g_top_output_dir = sys.argv[3]
    ...
    parallel_process_pool(g_top_rawdata_dir,g_top_v3v2_dir,g_top_output_dir,num_tasks)
    print 'refine finish'
