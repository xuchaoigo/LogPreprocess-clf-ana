...
def get_md5_list_from_file(server_branch_result_file,target_level):
    ...
    for line in handle:
        ...
        if int(arg_list[1]) == target_level:
            md5_list.append((arg_list[0],arg_list[2],arg_list[4]))
    return md5_list
...
def get_req(md5_list):
    md5_string=''
    for (md5,distance,density_string) in md5_list:
        md5_string='%s\n%s'%(md5_string,md5)
    postdata=urllib.urlencode({
        'opt':'0',
        'md5':md5_string,
...
def distance_large_enough(distance,density_string):
    density_list = density_string.split(' ')
    return True

    if int(density_list[6]) <= 0:
        return False
...
def process(server_branch_result_file):
    ...
    for table_name in md5_dict_lv200:
        ...
        while begin < table_md5_num:
            ...
            for i in range(end-begin):
                ...
                distance,density_string = get_param_from_mysql_list(html_parser.level[i_md5], md5_list_lv200)
                if distance ==142857:
                    print 'MD5 NOT FOUND!'
                ###########for print###########
                density_list = density_string.split(' ')
                this_density = density_list[6]
                #############################
                if html_parser.level[i_lv] == '100' or html_parser.level[i_lv] == '110':
                    vdc_white +=1
                    if distance_large_enough(distance,density_string):
                        distance_fp.append(distance)
                        density_fp.append(this_density)
    ...
    x=arange(0,len(distance_detect),1)
    plot(x,distance_detect,color='g',linewidth=1.0)
    xlabel("sample")
    ylabel("distance_detect")
    grid(True)

    ax = fig.add_subplot(222)
    x=arange(0,len(distance_fp),1)
    plot(x,distance_fp,color='r',linewidth=1.0)
    xlabel("sample")
    ylabel("distance_fp")
    grid(True)

    ax = fig.add_subplot(223)
    x=arange(0,len(density_detect),1)
    plot(x,density_detect,color='g',linewidth=1.0)
    xlabel("sample")
    ylabel("density_detect")
    grid(True)

    ax = fig.add_subplot(224)
    x=arange(0,len(density_fp),1)
    plot(x,density_fp,color='r',linewidth=1.0)
    xlabel("sample")
    ylabel("density_fp")
    grid(True)
    show()
    savefig('figure_of_vdc20w.png')
