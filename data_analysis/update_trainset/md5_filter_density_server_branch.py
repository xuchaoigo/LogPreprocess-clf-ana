        ...
        while begin < table_md5_num:
            ...
            for i in range(end-begin):
                try: 
                    i_md5 = 2*i
                    i_lv = 2*i+1
                    if i_md5>=len(html_parser.level):
                        print 'i_md5=%d,len of level=%d,conitnue'%(i_md5,len(html_parser.level))
                        continue    
                    #print 'md5=%s level=%s'%(html_parser.level[i_md5],html_parser.level[i_lv])
                    distance,quality_string = get_param_from_mysql_list(html_parser.level[i_md5], md5_list_lv200)
                    if distance ==142857:
                        print 'MD5 NOT FOUND!'
                    if html_parser.level[i_lv] == '100' or html_parser.level[i_lv] == '110':
                        vdc_white +=1
                    ...
                    print traceback.format_exc()
                    print 'i_md5=',i_md5
                    print 'len of level=',len(html_parser.level)
                    print 'len of list=',len(md5_list_lv200)
                    print 'md5=',html_parser.level[i_md5]
            html_parser.level=[]
            total_num += (end-begin)
            if total_num%500 ==0:
                print 'total process %d'%total_num
