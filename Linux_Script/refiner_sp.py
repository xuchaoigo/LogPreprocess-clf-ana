...
def CopyFiles(SrcDir, DstDir):
    ...
    for f in os.listdir(SrcDir):
        ...
        if os.path.isfile(SrcFile):
            ...
            if cnt%5000==0:
                print 'finish ',cnt
            PPOneLog(SrcFile,DstFile)                   
        elif os.path.isdir(SrcFile):
            if not os.path.exists(DstFile):
                os.makedirs(DstFile)
            CopyFiles(SrcFile, DstFile)

def PPOneLog(SrcFile,DstFile):
        ...
        while 1:
                ...
                if SamplePath == '':
                    if len(ParamList) >= 4 and len(ParamList[3][ParamList[3].rfind('\\')+1:])==36:#confirm this line actually contains md5
                        SamplePath = ParamList[3]
                        #print 'get SamplePath=',SamplePath
                    else:
                        continue
...
if __name__ == '__main__':
    ...
    st=time.clock()
    CopyFiles(arawdata_dir,output_dir)
    et=time.clock()
