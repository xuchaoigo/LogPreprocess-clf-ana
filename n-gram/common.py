...
def FetchFeature_whole(db_ip,db_name):
    global MySqlCon
    global MySqlCur
    InitCursor(db_ip)
    g_feature_dict={}   
...
def update_fet_width(table_name,index,new_BW,new_WW):
    global MySqlCon
    global MySqlCur
    cmd='update %s set black_count=%d, white_count=%d where Feature_Index=%d'%(table_name,int(new_BW),int(new_WW),int(index))
    #print cmd    
    MySqlCur.execute(cmd)
    MySqlCon.commit()

def get_fet_num_of_gft(db_ip,db_name):
    global MySqlCon
    global MySqlCur
    InitCursor(db_ip)
    SelectDB(db_name)
