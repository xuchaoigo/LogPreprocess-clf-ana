...
def do_mergefet_worker(job_q, result_q, sftp, output_incremental_fet, db_ip,db_name):
    print 'process %d start working...'%(os.getpid())
        ...
        try:
            ...
            for remote_fet_file in job:
                ...
                if robust_sftp(sftp,remote_fet_file,local_fet_file,True) == False:
                    continue
                try:
                    MergeFile_single.merge_fet_file(task_feature,local_fet_file,remote_fet_file)
                except Exception,ex:
                    print ex
                    print traceback.format_exc()
...
def process_file_worker(job_q, result_q, server_ip, operation, user_name, db_ip, db_name, ngram):
    ...
    if operation == OP_PREPROCESS:
        ...
        do_mergefet_worker(job_q,result_q,sftp,False,db_ip,db_name) 
    elif operation == OP_MERGEFET_INC:
        do_mergefet_worker(job_q,result_q,sftp,True,db_ip,db_name) 
    scp.close()
...
def run_client():
    if len(sys.argv) != 9:
        ...
        print 'supported operations: PREPROCESS,MIST2VECTOR,MERGEFET,MERGETABLE(ingore num_procs),REMAINTABLE,MERGEFET_INC'
        sys.exit()
