...
def _get_samplename_list():
    ...
    name_list.append('P2P_Worm')
    name_list.append('Trojan_Clicker')
    name_list.append('Trojan_Downloader')
    name_list.append('Trojan_Dropper')
    name_list.append('Trojan_PSW')
    name_list.append('Trojan_Spy')
    return name_list
        ...
        for name in files:
            ...
            if operation == OP_PREPROCESS:
                ...
                if not os.path.exists(fet_dir):
                    os.makedirs(fet_dir)
            elif operation == OP_MIST2VECTOR or operation == OP_MERGEFET or operation == OP_MERGEFET_INC:
                name_found = False
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print 'Usage:'
        print sys.argv[0], ' operation[%s|%s|%s|%s|%s], ...'%(OP_PREPROCESS,OP_MIST2VECTOR,OP_MERGEFET,OP_MERGETABLE,OP_MERGEFET_INC)
        sys.exit()
    ...
    if operation == OP_PREPROCESS:
        ...
        if len(sys.argv) != 5:
            ...
            sys.exit()
    elif operation == OP_MERGEFET_INC:
        if len(sys.argv) != 5:
            print 'Usage:'
            print sys.argv[0],' MERGEFET_INC dir_of_fet dir_of_feature listen_port'
            print 'dir_of_fet: must be FULL directory, and contains ',common.g_sample_fet,'!'
            print 'dir_of_feture: must be FULL directory, stores seperate feature files generatored by clients.'
            sys.exit()
    elif operation == OP_MERGETABLE:
